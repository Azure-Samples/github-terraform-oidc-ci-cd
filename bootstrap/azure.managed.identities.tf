...
locals {
  ...
  federated_credentials = { for federated_credential in flatten([for env_key, env_value in local.environment_split : [
    for template in env_value.required_templates : {
      ...
      subject                           = "repo:${var.organization_name}/${local.resource_names.repository_main_name}:environment:${env_key}:job_workflow_ref:${format(local.template_claim_structure, template)}"
    }
